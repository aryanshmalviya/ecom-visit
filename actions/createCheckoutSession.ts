...
export async function createCheckoutSession(items: GroupedBasketItem[], metadata: Metadata) {
    try {
        ...
        if (itemsWithoutPrice.length > 0) {
            throw new Error("Some items don't have prices on them !")
        }
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            success_url: `${`https://${process.env.VERCEL_URL}` || process.env.NEXT_PUBLIC_BASE_URL}/success?session_id={CHECKOUT_SESSION}&orderNumber=${metadata.orderNumber}`,
